...
1. **【修正済み・2026-08-22】「試薬登録ウィンドウの3Dビューで回転すると
   見かけの大きさが変わって見える」という報告に対応した。** 実機での
   再テストで、直径基準のスケール修正だけでは解消していないことが判明し、
   残っていた本当の原因(候補(b))を特定・修正した:
   - `compute_geometry`(`core/lineart_render.py`)の`max_bond_px`(小分子の
     過拡大防止キャップ)判定が、投影後(xy、回転後にzを捨てた座標)の結合長
     `proj_bond`を使っていた。結合が視線方向に近づくほどこの投影長は0に
     潰れるため、回転角によってキャップの発動有無が切り替わり、スケールが
     連続的に変化していた(直径基準にした最初の修正はここを直し忘れていた)。
   - 修正: `scene.bond_length`(3D座標・回転前から計算済みで回転に依存しない、
     元々「スケール決定に使う」とコメントされていたが実際には未使用だった
     フィールド)を使うように変更。孤立した3原子(A-B: 5Å、A-C: 1Å)のscene を
     手動構築し、y軸回転でA-Bが視線方向に近づいてもA-Cのラベル位置ピクセル
     間隔が完全に一定であることを確認する回帰テストを追加した
     (`tests/test_lineart_render.py::TestComputeGeometryScaleIsRotationInvariant`)。
     修正前のロジックで同じテストを流すと角度0°→89°でスケールが約2.1倍に
     増大することを検証済み(=ユーザー報告の「回転でズームして見える」と一致)。
   - 候補(a)(非球状分子の投影後の見かけの大きさが視点角度で変わるのは物理的に
     正しい挙動)は、これとは別に依然として真。今回の修正は「スケール
     (Å→pxの変換係数)が回転で変わらないこと」だけを保証するもので、
     分子のシルエット自体が向きで変わって見えること自体は仕様通り。
   - 全61件(`test_lineart_render.py`)含め、3D関連テストスイート
     (`test_structure.py`/`test_scene_builder.py`/`test_structure_3d.py`/
     `test_molecule_3d_view.py`/`test_structure_3d_tab.py`/
     `test_reagent_editor_dialog.py`/`test_library_dialog.py`/
     `test_main_window.py`)を個別実行しすべてパス。
   - **次のセッションでは、まずユーザーに実機で再確認してもらうこと**
     (今回はコード上の検証のみで、実機での見た目確認はまだ)。
2. macOS版は引き続き実機未検証(CIビルド成功のみ)。実機が手に入れば
   `.icns`アイコン反映も含めて動作確認すること。

## 新機能の実装方針(改善提案書§5、仕様書11章のTier A/B/C)

候補一覧は仕様書11章にあるが、優先順位や着手の仕方までは決めていなかった
ので、今回の基盤整備を踏まえた具体的な方針をここに残す。

### 推奨着手順

1. **`render_mode`手動切替UI(A1)を最初にやるのが良い。**
   `LibraryEntry.render_mode`列は元々存在しており(v2で追加済み)、今回の
   `renderer_version`自動焼き直し機構(仕様書4.7節)と組み合わせれば
   「モードを変更→即座に`preview_svg`を焼き直して保存」が自然に実装できる
   (`_ensure_current_preview`と同じパターンを流用できる)。UIを足すだけで
   完結し、コストも0.5〜1人日と見積もり通り小さい。分子構造描画の見た目に
   関する不満はこのセッションでほぼ解消したはずだが、それでも「自動判定と
   違う見た目にしたい」というケースの受け皿になる。
2. **Undo/Redo(B10)は「必要か」を先にユーザーへ再確認すること。**
   改善提案書は「着手するなら早い段階で(後から入れるほど高コストになる)」
   と明記している。今回3Dクリーンアップ機能に簡易Undo(直前1状態のみ)を
   実装したが、これは3Dタブに閉じたローカルな仕組みで、テーブル全体の
   Undo/Redoとは別物。バックログの見直し(グローバル指示CLAUDE.md #15)の
   観点からも、「実際に困っているか」をまず聞く方が先。困っていれば
   他のB tier機能より優先すべき性質のもの。
3. **InChIKey重複検出(A8)に着手する場合はDBマイグレーションが必要。**
   改善提案書はversion 3(molblock追加時)に相乗りする想定だったが、今回は
   YAGNI判断でinchikey列を追加しなかった(§2フェーズ3のふりかえり参照)。
   着手時は新規に`version 5`のマイグレーションを`db/schema.py`に追加する。
   RDKitの`Chem.MolToInchiKey`は標準機能なので計算コスト自体は低い。
4. それ以外のTier A(A2〜A7)は相互に独立なので、優先度に強いこだわりが
   なければユーザーの直近のニーズで選んでよい。
5. Tier B・Cは仕様書11章の記載通りスコープが膨らみやすいものが多い
   (特にB9在庫管理)。着手前に要件を絞る一手間を惜しまないこと。

### 進め方の運用方針(今回のセッションで機能した進め方)

- **フェーズごとにコスト・リスク・回帰対象を明示したロードマップを
  アーティファクトとして作り、完了のたびに更新する**、というやり方が
  今回の6フェーズでうまく機能した(ユーザーが進捗を一目で追える)。
  新機能でも同じパターンを踏襲するとよい。
- **設計判断が改善提案書やドキュメントの原案と食い違うとわかった場合は、
  黙って直さず「原案はこうだったが実測でこう変えた」という経緯を
  ロードマップ・仕様書双方に残す。** 今回はこれを徹底したことで、
  後から見返したときに「なぜこの実装なのか」が追える状態になっている。
- 大きな設計判断(DBスキーマ変更、既存機能の削除・大幅変更など)は
  着手前に選択肢を提示して確認する(グローバル指示CLAUDE.md #14)。
  一方、ユーザーは推奨案を1つに絞って提示すると即決してくれることが
  多い(過去の記憶`feedback-molweigh-collab-style`参照)。
- UIの見た目に関わる変更は、可能ならモックアップやスクリーンショットで
  先に確認するとすり合わせが早い。
